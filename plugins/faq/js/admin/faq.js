intelli.faq = function()
{
	var vUrl = 'controller.php?plugin=faq';
...
intelli.exGModel = Ext.extend(intelli.gmodel,
{
	...
	setupColumnModel: function()
	{
		...
		expander,
		{
			header: intelli.admin.lang.question, 
			dataIndex: 'question',
			id: 'question_column',
			sortable: true, 
			width: 250,
			...
			editor: new Ext.form.ComboBox({
				...
				editable: false,
				lazyRender: true,
				store: intelli.faq.statusesStore,
...
intelli.exGrid = Ext.extend(intelli.grid,
{
	...
	init: function()
	{
		...
		this.title = intelli.admin.lang.manage_faq;
		this.renderTo = 'box_faq';
		...
		this.grid.autoExpandColumn = 'question_column';
	...
	{
		this.dataStore.baseParams = {action: 'get', id: '0'};
	},
	...
	setEvents: function()
	{
		...
		/* Edit fields */
		intelli.faq.oGrid.grid.on('afteredit', function(editEvent)
		{
		...
		intelli.faq.oGrid.grid.on('cellclick', function(grid, rowIndex, columnIndex)
		{
			...
			if('edit' == fieldName)
			{
				intelli.faq.oGrid.saveGridState();

				window.location = intelli.faq.vUrl +'&do=edit&id='+ record.json.id;
			}
			...
			if('remove' == fieldName)
			{
				Ext.Msg.show(
				{
					...
					msg: intelli.admin.lang.are_you_sure_to_delete_this_faq,
		...
		intelli.faq.oGrid.grid.getSelectionModel().on('rowselect', function()
		{
		...
		intelli.faq.oGrid.grid.getSelectionModel().on('rowdeselect', function(sm)
		{
			if(0 == sm.getCount())
			{
		...
		{
			var rows = intelli.faq.oGrid.grid.getSelectionModel().getSelections();
		...
		Ext.getCmp('removeBtn').on('click', function()
		{
			...
			{
				title: intelli.admin.lang.confirm,
				msg: (ids.length > 1) ? intelli.admin.lang.are_you_sure_to_delete_this_faqs : intelli.admin.lang.are_you_sure_to_delete_this_faq,
				buttons: Ext.Msg.YESNO,
				icon: Ext.Msg.QUESTION,
				fn: function(btn)
				{
		...
		{
			intelli.faq.oGrid.grid.getStore().lastOptions.params.limit = new_value;
			intelli.faq.oGrid.grid.bottomToolbar.pageSize = parseInt(new_value);
...
Ext.onReady(function()
{
	if(Ext.get('box_faq'))
	{
		intelli.faq.oGrid = new intelli.exGrid({url: intelli.faq.vUrl});

		/* Initialization grid */
		intelli.faq.oGrid.init();
	}
	
	if(Ext.get('box_faq_categories'))
	{
		intelli.faq_categories.oGridCat = new intelli.exGridCat({url: intelli.faq_categories.vUrl});

		/* Initialization grid */
		intelli.faq_categories.oGridCat.init();
	}
});