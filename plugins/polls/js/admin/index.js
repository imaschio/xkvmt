...
{
	var vUrl = 'controller.php?plugin=polls';
...
intelli.exGModel = Ext.extend(intelli.gmodel,
{
	...
	setupColumnModel: function()
	{
		...
		this.checkColumn,
		{
			...
			header: intelli.admin.lang.expire_date, 
			dataIndex: 'expires',
			width: 130,
			sortable: true,
			editor: new Ext.form.DateField(
			...
			editor: new Ext.form.ComboBox({
				...
				editable: false,
				lazyRender: true,
				store: intelli.polls.statusesStore,
...
intelli.exGrid = Ext.extend(intelli.grid,
{
	...
	init: function()
	{
		...
		this.title = intelli.admin.lang.manage_polls;
		this.renderTo = 'box_polls';
		...
		this.bottomToolbar = new Ext.PagingToolbar(
		{
			...
			items: [
				...
				intelli.admin.lang.items_per_page,
				{
					...
					lazyRender: true,
					width: 80,
					store: intelli.polls.pagingStore,
					value: '10',
					displayField: 'display',
					valueField: 'value',
					mode: 'local',
	...
	setEvents: function()
	{
		...
		/* Edit fields */
		intelli.polls.oGrid.grid.on('afteredit', function(editEvent)
		{
			editEvent.value = Ext.isDate(editEvent.value) ? editEvent.value.format("Y-m-d H:i:s") : editEvent.value;

			Ext.Ajax.request(
			{
				waitMsg: intelli.admin.lang.saving_changes,
				url: intelli.polls.vUrl,
				method: 'POST',
				params:
				{
					action: 'update',
		...
		intelli.polls.oGrid.grid.on('cellclick', function(grid, rowIndex, columnIndex)
		{
			...
			if('edit' == fieldName)
			{
				intelli.polls.oGrid.saveGridState();

				window.location = 'controller.php?plugin=polls&do=edit&id='+ record.json.id;
			}
		...
		intelli.polls.oGrid.grid.getSelectionModel().on('rowselect', function()
		{
		...
		intelli.polls.oGrid.grid.getSelectionModel().on('rowdeselect', function(sm)
		{
			if(0 == sm.getCount())
			{
		...
		{
			var rows = intelli.polls.oGrid.grid.getSelectionModel().getSelections();
			...
			Ext.Ajax.request(
			{
				...
				success: function(data)
				{
					intelli.polls.oGrid.grid.getStore().reload();
		...
		{
			intelli.polls.oGrid.grid.getStore().lastOptions.params.limit = new_value;
			intelli.polls.oGrid.grid.bottomToolbar.pageSize = parseInt(new_value);
...
{
	if(Ext.get('box_polls'))
	{
		intelli.polls.oGrid = new intelli.exGrid({url: intelli.polls.vUrl});

		/* Initialization grid */
		intelli.polls.oGrid.init();
	}
});