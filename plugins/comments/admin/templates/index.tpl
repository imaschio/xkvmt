...
{if isset($smarty.get.do) && $smarty.get.do == 'edit'}
	...
	<form action="controller.php?plugin=comments&amp;do={$smarty.get.do}{if $smarty.get.do == 'edit'}&amp;id={$smarty.get.id}{/if}" method="post">
	{preventCsrf}
	...
	<tr>
		...
		<td>
			<select name="status">
				<option value="inactive" {if isset($comment.status) && $comment.status == 'inactive'}selected="selected"{/if}>{$esynI18N.inactive}</option>
				<option value="active" {if isset($comment.status) && $comment.status == 'active'}selected="selected"{/if}>{$esynI18N.active}</option>
			</select>
		</td>
	</tr>
	<tr class="all">
...
{include_file js="js/intelli/intelli.grid, js/ckeditor/ckeditor, js/ext/plugins/bettercombobox/betterComboBox, js/ext/plugins/panelresizer/PanelResizer, js/ext/plugins/progressbarpager/ProgressBarPager, plugins/comments/js/admin/comments"}
