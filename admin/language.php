...
if (isset($_GET['do']))
{
	...
	if ('delete' == $_GET['do'])
	{
		if (array_key_exists($_GET['language'], $esynAdmin->mLanguages))
		{
			...
			esynMessages::setMessage($esynI18N['language_deleted']);
...
if (isset($_POST['do']))
{
	if ('add_lang' == $_POST['do'])
	{
		...
		if (!$error)
		{
			$all_fields = $esynLanguage->all("`key`,`value`,`category`", "`code` = '".IA_LANGUAGE."'");
...
if (isset($_GET['action']))
{
	...
	if ('get' == $_GET['action'])
	{
		...
		if ('compare' == $_GET['grid'])
		{
			if (!empty($sort) && !empty($dir))
			{
				$order = "GROUP BY `key` ORDER BY `{$sort}` {$dir} LIMIT {$start}, {$limit}";
			}
			...
			if (isset($_GET['key']) && !empty($_GET['key']))
			{
				$conds[] = "`key` LIKE '%{$_GET['key']}%' ";
			}

			if (isset($_GET['value']) && !empty($_GET['value']))
			{
				$conds[] = "`value` LIKE '%{$_GET['value']}%' ";
			}
			...
			if (isset($_GET['filter_plugin']) && !empty($_GET['filter_plugin']))
			{
				$conds[] = "`plugin` = '{$_GET['filter_plugin']}' ";
			}

			$where = empty($conds) ? '1=1 ' : implode(' AND ', $conds);

			$esynAdmin->setTable('language');
			$rows = $esynAdmin->all("SQL_CALC_FOUND_ROWS DISTINCT `key`, `category`", $where.$order, array());
			$esynAdmin->resetTable();

			$out = array('data' => array(), 'total' => $esynAdmin->foundRows());
			...
			{
				$stmt = "`code` = '{$code}' AND `key` IN('" . implode("','", $keys) . "')";
				$lang[] = $esynAdmin->keyvalue("`key`, `value`", $stmt, array());
			}
			$esynAdmin->resetTable();
			...
			foreach ($rows as $row)
			{
				...
				{
					$keyName = 'lang_'.$code;
					$tmp[$keyName] = isset($lang[$i][$key]) ? $lang[$i][$key] : null;
					$i++;
				}
