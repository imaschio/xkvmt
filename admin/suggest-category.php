...
{
	define('IA_REALM', "edit_category");
}
else
{
	define('IA_REALM', "create_category");
}

define("IA_CATEGORY_ICONS_DIR", IA_UPLOADS . 'category-icons'.IA_DS);
define("IA_CATEGORY_ICONS_COMMON_DIR", IA_INCLUDES.'common'.IA_DS.'category-icons'.IA_DS);
...
if (isset($_POST['save']))
{
	...
	$new_category['hidden']				= isset($_POST['hidden']) ? (int)$_POST['hidden'] : 0;
	 ...
	 *
	 */
	if (isset($_POST['parent_id']) && ctype_digit($_POST['parent_id']))
	{
	...
	if (isset($_GET['do']) && 'edit' == $_GET['do'])
	{
		if (isset($_POST['parent_id']) && $category['parent_id'] != $_POST['parent_id'])
		{
			$parents = array();
	...
	{
		$_POST['path'] = esynUtil::getAlias($_POST['path']);
	...
	{
		$titlepath = esynUtil::getAlias($_POST['title']);
	...
	{
		$new_category['path'] = esynUtil::getAlias($new_category['path']);
	}
	/**
...
$esynSmarty->assign('columns_num_array', $columns_num_array);
