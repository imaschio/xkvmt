...
{if $smarty.get.view == 'language'}
	...
	<tr>
		...
		<td>
		{if $langs|count != 1 && $code != $config.lang}
...
{elseif $smarty.get.view == 'phrase'}
	<div id="box_phrases" style="margin-top: 15px;"></div>
{elseif $smarty.get.view == 'download'}
	{include file='box-header.tpl' title=$gTitle}
	...
	<tr>
		...
		<td>
			<select name="file_format">
				<option value="csv" {if isset($smarty.post.file_format) && $smarty.post.file_format == 'csv'}selected="selected"{/if}>{$esynI18N.csv_format}</option>
				<option value="sql" {if isset($smarty.post.file_format) && $smarty.post.file_format == 'sql'}selected="selected"{/if}>{$esynI18N.sql_format}</option>
			</select>
		</td>
	</tr>
	<tr>
	...
	{include file='box-footer.tpl'}
{elseif $smarty.get.view == 'add_lang'}
	...
	<tr>
		...
		<td>
		{foreach from=$langs key=code item=language}
			<b>{$language}</b>&nbsp;{if $code == $config.lang}[ {$esynI18N.default} ]{/if}<br />
		{/foreach}
		</td>
	</tr>
	...
	{include file='box-footer.tpl'}
{elseif $smarty.get.view == 'compare'}
	<div id="box_compare" style="margin-top: 15px;"></div>
{/if}

{include_file js="js/intelli/intelli.grid, js/ext/plugins/bettercombobox/betterComboBox, js/ext/plugins/panelresizer/PanelResizer, js/ext/plugins/progressbarpager/ProgressBarPager, js/admin/language"}
