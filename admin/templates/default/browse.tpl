			...
			<div class="last">
				<div class="category">
					{if $config.categories_icon_display}
						{if $value.icon}
							<img src="{$smarty.const.IA_URL}{$value.icon}" alt="" width="32" height="32" align="left" style="margin-right: 10px;"/>
						{elseif $config.default_categories_icon}
							<img src="{$smarty.const.IA_URL}{$config.default_categories_icon}" alt="" width="32" height="32" align="left" style="margin-right: 10px;"/>
						{else}
							<img src="{$smarty.const.IA_URL}templates/common/img/category_icons/category_icon.gif" alt="" width="32" height="32" align="left" style="margin-right: 10px;"/>
						{/if}
					{/if}
	...
	<div class="categories">
		...
		{foreach from=$related_categories key=key item=value}
			...
			{if !($cnt % 3) || $cnt == $related_categories|@count}
				<div class="last">
					<div class="category">
						<a href="controller.php?file=browse&amp;id={$value.id}" class="{$value.status}">{$value.title|escape:'html'}</a>
							&nbsp;{if $config.num_listings_display}({$value.num_all_listings}){/if}
							&nbsp;<a href="#" class="actions_edt-related_{$value.id}">{print_img full=true fl="icons/edit-grid-ico.png" admin=true style="vertical-align: middle;"}</a>
							&nbsp;<a href="#" class="actions_rmv-related_{$value.id}"><img style="vertical-align: middle;" src="{print_img fl="remove-grid-ico.png" folder="icons/" admin="true"}" alt="{$esynI18N.remove} {$value.title|escape:'html'}">
						</a>
					{if $config.subcats_display}
						{if isset($value.subcategories) && !empty($value.subcategories)}
							<div class="subcategories">
...
{include_file js="js/intelli/intelli.grid, js/ext/plugins/bettercombobox/betterComboBox, js/ext/plugins/rowexpander/rowExpander, js/ext/plugins/panelresizer/PanelResizer, js/ext/plugins/progressbarpager/ProgressBarPager, js/admin/browse, js/utils/dutil"}
