	...
	setupColumnModel: function()
	{
		...
		this.checkColumn,
		{
			...
			renderer: function(value, p, record)
			{
				...
				{
					value = Date.parseDate(value, "Y-m-d H:i:s").format(intelli.config.date_format_js);
				}
...
Ext.onReady(function()
{
	 ...
	 */
	var all_pages_count = $("input[name^='permissions']").length;
	var checked_pages_count = $("input[name^='permissions']:checked").length;

	if(checked_pages_count > 0 && all_pages_count == checked_pages_count)
	{
	...
	$("input[name^='permissions']").each(function()
	{
		$(this).click(function()
		{
			...
			var all_group_count = $("input." + group_class + '[name="permissions[]"]').length;
			var all_group_checked = $("input." + group_class + '[name="permissions[]"]:checked').length; 
			var checked = (all_pages_count == $("input[name^='permissions']:checked").length) ? 'checked' : '';

			if (all_group_count == all_group_checked)
			{
				//$('input[name="select_all_' + group_class + '"]').attr('checked', 'checked');
				$('input[name="select_all_' + group_class + '"]').prop('checked', 'checked');
			}
			else
			{
				//$('input[name="select_all_' + group_class + '"]').attr('checked', '');
				$('input[name="select_all_' + group_class + '"]').prop('checked', '');
			}
	...
	$("#permissions input[name^='select_all_']").each(function()
	{
		var group_class = $(this).attr("class");
		var all_group_count = $("input." + group_class + '[name="permissions[]"]').length;
		var all_group_checked = $("input." + group_class + '[name="permissions[]"]:checked').length; 
		...
		$(this).click(function()
		{
			...
			var all_checked = (all_pages_count == $("input[name^='permissions']:checked").length) ? 'checked' : '';
