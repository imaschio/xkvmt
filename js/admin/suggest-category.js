...
Ext.onReady(function()
{
	$("#change_category").click(function()
	{
		if(!intelli.suggestCategory.categoriesTree)
		{
			...
			new Ext.tree.TreeSorter(intelli.suggestCategory.categoriesTree, {folderSort: false});
	...
	$("#choose_icon").click(function()
	{
		if(!intelli.suggestCategory.chooser)
		{
    		intelli.suggestCategory.chooser = new ImageChooser(
			{
				...
				root_url: intelli.config.esyn_url,
    			width: 515,
    			height: 350
    		});
    	}
		...
		intelli.suggestCategory.chooser.show(this, function(data)
		{
			Ext.DomHelper.overwrite('icons', {
				tag: 'img', src: intelli.config.esyn_url + data.url, style:'margin:10px;visibility:hidden;'
	...
	{
		var display = (1 == $(this).val()) ? 'show' : 'hide'; 

		$('#confirmation_text')[display]();
	});
