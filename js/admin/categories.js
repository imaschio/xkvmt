...
intelli.exGModel = Ext.extend(intelli.gmodel,
{
	...
	setupColumnModel: function()
	{
		...
		expander, {
			...
			header: intelli.admin.lang.level,
			dataIndex: 'level',
			sortable: true,
			width: 60
		},{
...
intelli.exGrid = Ext.extend(intelli.grid,
{
	...
	constructor: function(config)
	{
		...
		this.dataStore.setDefaultSort('level');
	},
	init: function()
	{
	...
	setupBaseParams: function()
	{
		...
		var quick_search = null;
		if (intelli.urlVal('quick_search'))
		{
			quick_search = intelli.urlVal('quick_search').replace('+',' ');
		}
...
Ext.onReady(function()
{
	if(Ext.get('box_categories'))
	{
		...
		if(intelli.urlVal('quick_search'))
		{
			Ext.getCmp('searchTitle').setValue(intelli.urlVal('quick_search').replace('+', ' '));
		}
	}
});