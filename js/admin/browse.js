	...
	setEvents: function()
	{
		...
		Ext.getCmp('actBtn').on('click', function()
		{
			...
			if (intelli.inArray(action, ['copy', 'cross', 'move']))
			{
				...
				new Ext.tree.TreeSorter(intelli.browse.vTree, {folderSort: false});
	...
	$("a[class*='actions']").each(function()
	{
		$(this).click(function()
		{
			...
			if (intelli.inArray(params[1], ['related', 'crossed']))
			{
				...
				// add a tree sorter in folder mode
				new Ext.tree.TreeSorter(intelli.browse.vTreeM, {folderSort: false});
			...
			if ('edt-related' == params[1])
			{
				Ext.Msg.prompt('Related Category Title', 'Please enter category title:', function(btn, text)
				{
					if (btn == 'ok')
					{
						Ext.Ajax.request(
