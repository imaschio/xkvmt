...
class esynPlugin extends esynAdmin
{
	...
	/**
	 * @var string url_alias value for the plugin processing
	 */
	var $url = '';
	...
	function upgrade()
	{
		...
		if (!empty($this->phrases))
		{
			...
			$this->delete("`plugin` = '{$plugin['name']}' AND `code` = 'en' AND `category` != 'page'");
		...
		{
			$this->setTable('language');
			$this->delete("`key` LIKE 'admin_block_title_%' AND `plugin` = '{$plugin['name']}'");
			$this->resetTable();
			...
			foreach($this->adminBlocks as $block)
			{
				if (isset($block['title']) && !empty($block['title']))
				{
					...
					{
						$lang_content = array(
							"key"		=> 'admin_block_title_' . $block['name'],
							"value"		=> $title,
							"code"		=> $lngcode,
							"lang"		=> $lngvalue,
							"category"	=> 'admin',
		...
		if (!empty($this->adminPages))
		{
			...
			{
				// save admin page title
				$admin_page_titles[$page['aco']] = $page['title'];
			...
			if (!empty($admin_page_titles))
			{
				$this->setTable('language');
				$this->delete("`key` LIKE 'admin_page_title_%' AND `plugin` = '{$plugin['name']}'");

				foreach ($admin_page_titles as $name => $title)
				{
					foreach ($this->mLanguages as $lngcode => $lngvalue)
					{
						$lang_content = array(
							"key"		=> 'admin_page_title_'.$name,
							"value"		=> $title,
							"code"		=> $lngcode,
							"lang"		=> $lngvalue,
							"category"	=> 'admin',
		...
		if (!empty($this->config))
		{
			...
			foreach ($this->config as $config)
			{
				if ($this->exists("`name` = '{$config['name']}'"))
				{
					...
					$this->update($config, "`name` = '{$config['name']}'" ,array(), array("order" => $maxorder++));
				}
				else
				{
					$this->insert($config, array("order" => $maxorder++));
			...
			$this->delete("`name` NOT IN ('" . implode("','", $new_config) . "') AND `plugin` = '{$plugin['name']}'");
		...
		if (!empty($this->pages))
		{
			...
			foreach($this->pages as $page)
			{
				...
				if (isset($page['menus']))
				{
					if (!empty($page['menus']))
					{
						$menus = explode(',', $page['menus']);

						$this->setTable('block_pages');
						$this->delete("`page_name` = '{$page['name']}'");

						foreach ($menus as $menu)
						{
							$this->insert(array('block_name' => $menu, 'page_name' => $page['name']));
						}
		...
		/**
		 * Update new field groups
		 */
		if (!empty($this->field_groups))
		{
			$this->setTable('field_groups');
			...
			foreach ($this->field_groups as $group)
			{
				...
				if (!empty($group['name']))
				{
					...
					$this->setTable('field_groups');
					$exist = $this->exist('`name` = :name', array('name' => $group['name']));
					$this->resetTable();

					if ($exist)
					{
						$token = esynUtil::getNewToken(6);
						$new_group['name'] = 'fgroup_' . $token;
					}
				}
				else
				{
				...
				if (!empty($field_group_title))
				{
					...
					if (!utf8_is_valid($field_group_title))
					{
						$field_group_title = utf8_bad_replace($field_group_title);
					}
				}
				else
				{
					$field_group_title = $group['name'];
				}
				...
				if (!empty($group['pages']))
				{
					...
					{
						if (!in_array($page, array('suggest', 'edit', 'view', 'search')))
						{
							unset($group['pages'][$key]);
						}
					}

					$new_group['pages'] = implode(',', $group['pages']);
				}

				$new_group['plugin'] = $this->name;
				$new_group['order'] = $maxorder;

				$this->setTable('field_groups');
				$this->insert($new_group);
				$this->resetTable();
				...
				foreach ($this->mLanguages as $code => $lang)
				{
					...
					$insert['key'] = 'field_group_title_' . $new_group['name'];
					$insert['value'] = $field_group_title;
					$insert['lang'] = $lang;
					$insert['category'] = 'common';
					$insert['code'] = $code;
					$insert['plugin'] = $this->name;
		...
		if (!empty($this->upgradeSQL))
		{
			...
			foreach($this->upgradeSQL as $version => $sql)
			{
				if (version_compare($version, $installed_version, ">"))
				{
					foreach($this->upgradeSQL[$version] as $one_query)
					{
						if (!empty($one_query['query']))
						{
							$one_query['query'] = str_replace('{prefix}', $this->mPrefix, $one_query['query']);
							$one_query['query'] = str_replace('{mysql_version}', $mysql_ver_data, $one_query['query']);

							$this->query($one_query['query']);
						}
					}
				}
			}
	...
	function uninstall($plugin)
	{
		 ...
		 */
		$this->setTable('pages');
		$plugin_pages = $this->keyvalue('`id`, `name`', "`plugin` = '{$plugin}'");
		$this->resetTable();
		 ...
		 */
		$this->cascadeDelete(array("admin_blocks", "admin_pages", "language", "config_groups", "config", "pages", "hooks", "field_groups"), "`plugin` = '{$plugin}'");
	...
	function install()
	{
		...
		if (!empty($this->phrases))
		{
			//add config for email templates
			foreach ($this->phrases as $key => $phrase)
			{
				$pattern = "/^tpl_(.*?)_body$/i";
				preg_match($pattern, $phrase['key'], $preg_out);

				if (isset($preg_out[1]) && !empty($preg_out[1]))
				{
					$this->setTable("config");
					...
					$config = array();
					$config['group_name'] = 'mail';
					$config['name'] = $preg_out[1];
					$config['type'] = 'hidden';
					$config['multiple_values'] = "'0','1'";
					$config['value'] = '1';
					$config['plugin'] = $this->name;
					$config['order'] = $maxorder;
		...
		if (!empty($this->config))
		{
			...
			foreach($this->config as $config)
			{
				...
				$exist = $this->exists('`name` = :name', array('name' => $config['name']));
				if (!$exist)
				{
					$this->insert($config, array("order" => $maxorder));
					$maxorder++;
		...
		if (!empty($this->fields))
		{
			...
			foreach($this->fields as $field)
			{
				...
				$field['group'] = isset($field['group']) && !empty($field['group']) ? $field['group'] : '';
		...
		if (!empty($this->installSQL))
		{
			...
			foreach($this->installSQL as $sql)
			{
				if ($sql['external'])
				{
					$file_full_path = IA_PLUGINS . $this->name . IA_DS . $sql['external'];
				...
				if (!empty($sql['query']))
				{
					foreach($sql['query'] as $sql_query)
					{
						$sql_query = str_replace('{prefix}', $this->mPrefix, $sql_query);
	...
	function charData($parser, $text)
	{
		...
		if ('hook' == $this->inTag)
		{
			$this->hooks[] = array(
				...
				'page_type' => $this->_getAttr('page_type', ''),
				"plugin"	=> $this->name,
				"code"		=> $text,
				"file"		=> $this->_getAttr('file', ''),
				"status"	=> 'active',
				"order"		=> $this->_getAttr('order', '')
			);
		}
		...
		if (in_array('tooltips', $this->path) && 'tooltip' == $this->inTag)
		{
			$this->phrases[] = array(
				"key"		=> $this->attributes['key'],
				"value"		=> $text,
				"lang"		=> isset($this->attributes['lang']) ? $this->attributes['lang'] : $this->mLanguages[IA_LANGUAGE],
				"category"	=> 'tooltip',
				"code"		=> isset($this->attributes['code']) ? $this->attributes['lang'] : IA_LANGUAGE,
				"plugin"	=> $this->name
			);
		}
		...
		if (in_array('fields', $this->path) && 'field' == $this->inTag)
		{
			$this->fields[] = array(
				...
				"group"		=> isset($this->attributes['group']) ? $this->attributes['group'] : 'general',
		...
		/* Field groups */
		if (in_array('fieldgroups', $this->path) && 'group' == $this->inTag)
		{
		...
		if (in_array('installsql', $this->path) && 'sql' == $this->inTag)
		{
			$this->installSQL[] = array(
				'query'		=> array($text),
				'external'	=> $this->_getAttr('external', '')
			);
		}
		...
		// other fields
		if (in_array($this->inTag, array('version', 'summary', 'title', 'author', 'contributor', 'notes', 'status', 'date', 'compatibility', 'url')))
		{
