...
class esynListingField extends eSyndiCat
{
	 ...
	 * @access public
	 */
	var $readOnlyFields = array('title', 'url', 'description', 'reciprocal', 'email', 'meta_description', 'meta_keywords');
	 ...
	 * @return bool
	 */
	function insert($field, $addit = null)
	{
		...
		$this->startHook("beforeFieldInsert", $params);
		...
		$this->startHook("afterFieldInsert", $params);
	...
	function alterAdd($field)
	{
		...
		switch ($field['type'])
		{
			...
			case 'image':
				$sql .= "TEXT ";
				$sql .= "NOT NULL, ADD `{$field['name']}_title` TEXT ";
				break;
			case 'pictures':
				$sql .= "TEXT ";
				$sql .= "NOT NULL, ADD `{$field['name']}_titles` TEXT ";
				break;
			default:
				if ($field['values'])
				{
	...
	function delete($ids = '', $arg1 = array(), $arg2 = false)
	{
		...
		$this->startHook("beforeFieldDelete", $params);
		...
		$fields = $this->all("`id`, `name`, `type`", $where);

		if ($fields)
		{
			foreach($fields as $key => $field)
			{
				if (in_array($field['name'], $this->readOnlyFields))
				{
					$this->message = "The {$field['name']} field is read only.";

					return false;
				}
				else
				{
					parent::delete("`id` = :id", array('id' => $field['id']));

					parent::setTable("language");
					parent::delete("`key` LIKE 'field_" . $field['name'] . "_%'");
					parent::resetTable();

					parent::setTable("field_plans");
					parent::delete("`field_id` = :id", array('id' => $field['id']));
					parent::resetTable();
					...
					$listing_fields = parent::describe($this->mPrefix . 'listings');

					if ($listing_fields)
					{
						foreach ($listing_fields as $listing_field)
						{
							if ($listing_field['Field'] == $field['name'])
							{
								// remove uploaded files are related to listing field
								if (in_array($field['type'], array('image', 'storage', 'pictures')))
								{
									$this->setTable('listings');
									$field_values = $this->onefield("`{$field['name']}`", "`{$field['name']}` != ''");
									$this->resetTable();
									...
									if (!empty($field_values))
									{
										...
										{
											if ('pictures' == $field['type'])
											{
												$img_files = explode(',', $field_value);
											...
											{
												$img_path = IA_HOME . 'uploads' . IA_DS . $field_value;
												$img_small_path = IA_HOME . 'uploads' . IA_DS . 'small_' . $field_value;
								...
								$this->query("ALTER TABLE `{$this->mPrefix}listings` DROP `{$field['name']}`");

								if ('image' == $field['type'])
								{
									$this->query("ALTER TABLE `{$this->mPrefix}listings` DROP `{$field['name']}_title`");
								}

								if ('pictures' == $field['type'])
								{
									$this->query("ALTER TABLE `{$this->mPrefix}listings` DROP `{$field['name']}_titles`");
								}
		...
		$this->startHook("afterFieldDelete", $params);
	 ...
	 * @return bool
	 */
	function update($field, $where = '', $values = array(), $addit=null)
	{
		$vars = get_defined_vars();
		...
		$this->startHook("beforeFieldUpdate", $params);
		...
		if (isset($field['values']) && is_array($field['values']))
		{
			foreach($field['values'] as $key => $value)
      		{
				if (parent::one("id", "`key` = 'field_{$field['name']}_{$key}' AND `code` = '".IA_LANGUAGE."'"))
				{
					parent::update(array("value" => $value), "`key` = 'field_{$field['name']}_{$key}' AND `code` = '".IA_LANGUAGE."'");
				}
        		else
				{
					$phrase = array(
						...
						"value"		=> $value,
						"lang"		=> $this->mLanguages[IA_LANGUAGE],
						"code"		=> IA_LANGUAGE,
						"category"	=> 'common'
					);
		...
		$this->startHook("afterFieldUpdate", $params);
	...
	function alterEdit($field)
	{
		...
		switch ($field['type'])
		{
			...
			case 'image':
				...
				$exist = $this->fieldExist('listings', $field['name'].'_title');

				if (!$exist)
				{
					$c_sql = "ALTER TABLE `{$this->mPrefix}listings` "
						. "ADD `{$field['name']}_title` TEXT";
	...
	function fieldExist($table, $field)
	{
		...
		$fields = $this->describe($this->mPrefix . $table);
