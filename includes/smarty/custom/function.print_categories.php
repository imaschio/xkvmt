...
function smarty_function_print_categories($params, &$smarty)
{
	...
	global $esynCategory, $esynLayout;

	$aCols = (isset($params['aCols']) && 0 != $params['aCols']) ? (int)$params['aCols'] : (int)$esynConfig->getConfig('num_categories_cols');
	$aCols = (0 == $aCols) ? 1 : $aCols;
	...
	$categories_icon_display = $esynConfig->getConfig('categories_icon_display');

	// if default icon is not set
	if (empty($default_icon))
	{
		// check if exist category icon in current template
		$default_icon = IA_TEMPLATES . $esynConfig->getConfig('tmpl') . '/img/category_icons/category_icon.gif';

		// if icon doesn't exist take it from common folder
		if (!file_exists($default_icon))
		{
			$default_icon = 'templates/common/img/category_icons/category_icon.gif';
		}
		else
		{
			$default_icon = str_replace(IA_HOME, '', $default_icon);
		}
	}
		...
		foreach($params['aCategories'] as $key => $value)
		{
			...
			if('horizontal' == $display_type)
			{
				if (!($cnt % $aCols) || ($cnt == $categories_count))
				{
					...
					if ($categories_icon_display)
					{
						$out .= "<img src=\"{$icon}\" alt=\"".$category_title."\" {$dimension} />";
					}
					
					$out .= "<a title=\"{$category_full_title}\" href=\"{$url}\" class=\"countable categories\" id=\"ctg_{$value['id']}\"{$nofollow}>".$category_title."</a>{$crossed}<span>{$cause}</span></div>";
			...
			else
			{
				...
				$tmp .= "<a href=\"{$url}\" title=\"{$category_full_title}\" class=\"countable categories\" id=\"ctg_{$value['id']}\"{$nofollow}>".$category_title."</a>{$crossed}<span>{$cause}</span>";
			...
			if(!empty($vertical_categories))
			{
				for($i = 0; $i < $rows; $i++)
				{
					for($j = 1; $j <= $aCols; $j++)
					{
						...
						$out .= ($vertical_categories_count == $key || !($j % $aCols) || !isset($vertical_categories[$j][$i])) ? '</tr>' : '';
					}
				}
			}
		}
